...
describe("Test of component GifItem", () => {
  ...
  test("Must by math test with the snapshot of component GifItem", () => {
    const { container } = render(<GifItem title={title} url={url} />);
    expect(container).toMatchSnapshot();
  });

  test("Must by show the image and url indicate", () => {
    render(<GifItem title={title} url={url} />);
    expect(screen.getByRole("img").src).toBe(url);
  });

  test("Must by show the alt indicate to by image", () => {
    render(<GifItem title={title} url={url} />);
    expect(screen.getByRole("img").alt).toBe(title);
  });

  test("Must by show title of card component", () => {
    render(<GifItem title={title} url={url} />);
